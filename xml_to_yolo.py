...
def check_arguments_errors(args: argparse.Namespace) -> None:
    if not os.path.exists(args.input.txt):
        raise (ValueError(f"Invalid input.txt folder path: {os.path.abspath(args.input.txt)}"))
...
def main() -> None:
    ...
    xml_to_txt(args.input.txt, args.output)
